...
const Element: React.FC<Datas> = () => {
    ...
    useEffect(() => {
        ...
        const dateCheck: string = (String(currentDate.getFullYear()) + "-" + ("0" + String(currentDate.getMonth() + 1)).slice(-2) + "-" + ("0" + String(currentDate.getDate())).slice(-2));
        window.electron.getAllData().then(result => {
            setCliantJson(result);
            if (result[result.length - 1] !== undefined && dateCheck === result[result.length - 1].date) {
                setTodaysPomodoroCount(("0" + String(result[result.length - 1].count)).slice(-2));
            }
        })
    }, []);
