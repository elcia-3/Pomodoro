...
app.on("ready", async () => {
  ...
  function WindowResize() {
    ...
    if (NextWindowSize[1] < size.height - 100) {
      if (
        currentWindowSize[0] < NextWindowSize[0] ||
        currentWindowSize[0] > NextWindowSize[0]
      ) {
...
export const getAllData = (): POMODORODATA[] | boolean => {
  ...
  if (db.tableExists("PomodoroHistory", savePath)) {
    db.getAll(
      "PomodoroHistory",
      savePath,
      (success: boolean, contents: POMODORODATA[]) => {
        if (success) {
...
export const dbupdate = () => {
  ...
  const today: string =
    ...
    "-" +
    ("0" + String(currentDate.getMonth() + 1)).slice(-2) +
    "-" +
    ("0" + String(currentDate.getDate())).slice(-2);
  if (db.tableExists("PomodoroHistory", savePath)) {
    db.getRows(
      "PomodoroHistory",
      savePath,
        ...
        if (success) {
          ...
          if (contents.length > 0) {
            ...
            db.updateRow(
              ...
              { date: today },
              { count: newCount },
              (successupdaterow: boolean, message: string) => {
                if (successupdaterow) {
                  console.log(
                    "updatePomodoroHistoryData.updateRow success." + message
                  );
                } else {
                  console.log(
                    "updatePomodoroHistoryData.updateRow failed." + message
                  );
                }
              }
            );
